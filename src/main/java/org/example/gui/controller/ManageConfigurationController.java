...
public class ManageConfigurationController extends Controller {
  ...
  public void initialize() {
    try {
      ConfigurationDTO configurationDTO = CONFIGURATION_DAO.getOne("EVALUATION-ENABLED");

      if (configurationDTO == null) {
        CONFIGURATION_DAO.createOne(new ConfigurationDTO("EVALUATION-ENABLED", false));
        configurationDTO = CONFIGURATION_DAO.getOne("EVALUATION-ENABLED");
      }

      checkBoxEnableEvaluation.setSelected(configurationDTO.enabled());
    } catch (Exception e) {
  ...
  public void handleManageConfiguration() {
    try {
      CONFIGURATION_DAO.updateOne(new ConfigurationDTO("EVALUATION-ENABLED",checkBoxEnableEvaluation.isSelected()));
